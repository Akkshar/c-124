...
while True:
    try:
        ...
        pix_filter = 20
        min_pix = np.percentile(im_resized_inverted)
        im_resized_inverted_scaled = np.clip(im_resized_inverted_scaled-min_pix, 0, 255)

        max_pix = np.max(im_resized_inverted)
        im_resized_inverted_scaled = np.asarray(im_resized_inverted/max_pix)
        #print(im_resized_inverted_scaled)
        test_sample = np.clip(im_resized_inverted_scaled-min_pix, 0, 255).reshape(1,784)
